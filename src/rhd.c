...
static const uint16_t RHD_ADC_CH_CMD[32] = {
...
int rhd_send(rhd_device_t *dev, rhd_packet_t data)
{
  ...
  default:
  {
    uint16_t rx_buf[2] = {0};
    uint16_t tx_buf[2] = {0};
    int data_raw = rhd_duplicate_word(data.raw);

    dev->rw(tx_buf, rx_buf, 2);
    break;
  }
  }
}
...
int rhd_cfg_fs(rhd_device_t *dev, float fs, int n_ch)
{
  ...
  rhd_w(dev, SUPPLY_SENSOR_ADC_BUF_BIAS, adc_buf_bias_lut[i_lut]);
...
void rhd_sample_all(rhd_device_t *dev)
{
  ...
  {
    rhd_send_raw(dev, RHD_ADC_CH[ch]);
    int recvd_ch = ch < 2 ? 31 - ch : ch - 2;
    rhd_get_samples_from_rx(dev, recvd_ch);
  }
...
int rhd_duplicate_byte(uint8_t val)
{
  int out = 0;
  for (int i = 0; i < 8; i++)
  {
...
int rhd_duplicate_word(uint16_t val)
{
  int out = 0;
  for (int i = 0; i < 16; i++)
  {
